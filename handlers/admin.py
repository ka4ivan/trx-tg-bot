...
from config import dp, admins, bot, _
...
async def start_mailing(message: types.Message):
    ...
    await message.answer(_('👤 Адмін панель:                                                                        ㅤ'), reply_markup=markup_admin)
...
async def language(call: types.callback_query, state: FSMContext):
    ...
    all = types.InlineKeyboardButton(_('Всі користувачі'), callback_data="all_mailing")
    exit = types.InlineKeyboardButton(text=_('❌ Відмінити'), callback_data='quit')
    markup_language.add(ukrainian, russian, english).add(all).add(exit)
    await call.message.answer(_('🌬 Оберіть мову розсилки'), reply_markup=markup_language)


@dp.callback_query_handler(text='ua_mailing', state=mailing.state)
async def start(call: types.callback_query, state: FSMContext):
    users = await db_commands.get_user_id_by_language('uk')
    data = await state.get_data()
    text = data.get('text')
    for user in users:
        try:
            ...
            pass
    await state.finish()
    await call.message.answer(_('✅ Розсилка виконана для україномовних користувачів'))


@dp.callback_query_handler(text='ru_mailing', state=mailing.state)
async def start(call: types.callback_query, state: FSMContext):
    users = await db_commands.get_user_id_by_language('ru')
    data = await state.get_data()
    text = data.get('text')
    for user in users:
        try:
            ...
            pass
    await state.finish()
    await call.message.answer(_('✅ Розсилка виконана для російськомовних користувачів'))


@dp.callback_query_handler(text='en_mailing', state=mailing.state)
async def start(call: types.callback_query, state: FSMContext):
    users = await db_commands.get_user_id_by_language('en')
    data = await state.get_data()
    text = data.get('text')
    for user in users:
        try:
            ...
            pass
    await state.finish()
    await call.message.answer(_('✅ Розсилка виконана для англомовних користувачів'))


@dp.callback_query_handler(text='all_mailing', state=mailing.state)
async def start(call: types.callback_query, state: FSMContext):
    users = await db_commands.get_users()
    data = await state.get_data()
    text = data.get('text')
    for user in users:
        try:
            ...
            pass
    await state.finish()
    await call.message.answer(_('✅ Розсилка виконана для всіх користувачів'))
...
async def start(call: types.callback_query, state: FSMContext):
    ...
    for user in users:
        try:
            await dp.bot.send_audio(chat_id=user, audio=audio, caption=text)
            await sleep(0.33)
        except Exception:
            pass
    await state.finish()
...
async def mailing_photo(message: types.Message, state: FSMContext):
    ...
    photo = data.get('photo')
    audio = data.get('audio')
    markup = types.InlineKeyboardMarkup(row_width=2, inline_keyboard=[[types.InlineKeyboardButton(text=_('✅ Виконати розсилку'), callback_data='next'), types.InlineKeyboardButton(text='❌ Відмінити', callback_data='quit')]])
...
async def start(call: types.callback_query, state: FSMContext):
    ...
    for user in users:
        try:
            await dp.bot.send_photo(chat_id=user, photo=photo, caption=text)
            await dp.bot.send_audio(chat_id=user, audio=audio)
            await sleep(0.33)
        except Exception:
            pass
    await state.finish()
...
async def quit(call: types.callback_query, state: FSMContext):
    ...
    await call.message.answer(_('❌ Розсилка відмінена'), reply_markup=markup_start)
